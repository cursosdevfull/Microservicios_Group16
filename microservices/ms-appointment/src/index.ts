...
(async () => {
    try {
        ...
        const kafka = KafkaService.getInstance([env.KAFKA_TOPIC_PE, env.KAFKA_TOPIC_MX, env.KAFKA_TOPIC_CO, env.KAFKA_TOPIC_UPDATE_STATUS], env.KAFKA_GROUP_ID, KafkaBootstrap.getKafkaInstance());
        const listPromises = [kafka.connectProducer(), kafka.connectConsumer()];
